...
def safe_reservation_remaining(reserved_until: int, current_block: int) -> Optional[int]:
    ...
    Pinned to USER_POST_TX_CUSHION_BLOCKS, which mirrors the miner-side
    cushion. Inside this window validators cannot land a propose + challenge
    before the reservation expires — so a confirm broadcast here has no
    extension rescue path. On tao→btc the source TAO is sent to the miner
    before the swap row exists, making a failed reservation an
    unrecoverable loss.
    """
