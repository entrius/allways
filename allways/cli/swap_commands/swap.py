...
def poll_for_swap_creation(client, miner_hotkey: str, max_polls: int = 60) -> Optional[int]:
    """Poll contract until the miner's swap appears on-chain.

    Shows an elapsed-time counter so the wait does not look frozen. Warns
    after five consecutive RPC failures so a broken contract connection is
    ...
    with console.status('') as status:
        ...
        for i in range(max_polls):
            elapsed = i * 3
            status.update(
                f'[dim]Waiting for validators to confirm and initiate swap... '
                f'{elapsed // 60}:{elapsed % 60:02d} elapsed[/dim]'
            )
            time.sleep(3)
            try:
...
def wait_for_swap_initiation(client, miner_hotkey: str, miner_uid: int, max_polls: int) -> Optional[int]:
    ...
    """
    try:
        swap_id = poll_for_swap_creation(client, miner_hotkey, max_polls)
    except KeyboardInterrupt:
        try:
            swap_id = resolve_recent_swap_id(client, miner_hotkey)
        except ContractError:
...
def swap_now_command(
    ...
    swap_id = wait_for_swap_initiation(client, selected_pair.hotkey, selected_pair.uid, max_polls)
    if swap_id is None:
        return
