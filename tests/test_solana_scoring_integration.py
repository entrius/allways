...
Deploys the B3.0 program, activates two bound miners (A quotes a better btc→tao
rate than B), ingests the emitted events through ``SolanaEventIndex`` with real
sr25519 attribution, then checks the full B3.6 wiring against live on-chain
state: the index reconstructs the active set + collateral, ``build_eligibility``
reads the on-chain ``MinerState`` counters, the crown replay credits the
better-rate holder A, and a real ``calculate_miner_rewards`` round gates the
(0-success) miners to weight 0 — then, with A marked eligible, credits the
btc→tao pool to A while the non-crown miner B stays at 0.
...
def test_scoring_round_off_solana_events(env):
    ...
    hka, hkb = hk_a.ss58_address, hk_b.ss58_address
    ...
    assert index.get_active_miners_at(now) == {hka, hkb}
    assert index.get_miner_collaterals_at(now) == {hka: coll_a, hkb: coll_b}

    # ── crown replay credits the better-rate holder A, not B ──
    crown = replay_crown_time_window(
        store=store, event_index=index, from_chain='btc', to_chain='tao',
        window_start=now - 600, window_end=now + 60, rewardable_hotkeys={hka, hkb},
    )
    assert set(crown) == {hka}
    assert crown[hka] > 0

    # ── eligibility is read off the on-chain MinerState counters ──
    metagraph = _padded_metagraph([hka, hkb])
    eligibility = build_eligibility(admin, metagraph)
    # No completed swaps on localnet yet → 0 successes → both ineligible.
    assert eligibility.get(hka) is False and eligibility.get(hkb) is False
    ...
    assert rewards[0] == 0.0 and rewards[1] == 0.0  # A, B
    assert rewards[RECYCLE_UID] == pytest.approx(1.0, abs=1e-5)

    # Round 2 — patch A eligible: the btc→tao pool is credited to crown holder A;
    # B (lower rate, not a crown holder) earns nothing.
    with patch('allways.validator.scoring.build_eligibility', return_value={hka: True, hkb: True}):
