...
def make_active_swap(swap_id: int) -> Swap:
    return Swap(
        id=swap_id,
        user_hotkey='u',
        miner_hotkey='m',
...
class TestGetActiveSwapsMaxGap:
    """``max_gap=None`` must walk every ID; finite ``max_gap`` halts on a run
    of pruned (None) lookups so callers don't pay an O(N) scan by default."""

    def _make_client(self, swap_at_id: int, latest_id: int):
        client = AllwaysContractClient(contract_address='5xx', subtensor=make_subtensor())
        client.ensure_initialized = lambda: None
        client.get_next_swap_id = MagicMock(return_value=latest_id + 1)
        # One ACTIVE swap at swap_at_id; every other ID returns None (pruned).
        active = make_active_swap(swap_at_id)
        client.get_swap = MagicMock(side_effect=lambda sid: active if sid == swap_at_id else None)
        return client, active

    def test_default_max_gap_halts_before_reaching_distant_active_swap(self):
        client, _ = self._make_client(swap_at_id=1, latest_id=200)

        result = client.get_active_swaps()  # default max_gap=50

        assert result == []  # halted on 50 consecutive Nones, never reached id=1

    def test_max_gap_none_walks_to_first_id_and_finds_distant_swap(self):
        client, active = self._make_client(swap_at_id=1, latest_id=200)

        result = client.get_active_swaps(max_gap=None)

        assert result == [active]
        # 200 IDs walked: id 1 hit + 199 None lookups, no early break.
        assert client.get_swap.call_count == 200
